...
final class Admin extends BaseController
{
    public function index()
    {
        ...
        return view('admin/index', [
            ...
            'users' => $db
                ->table('users u')
                ->select('u.id,u.username,p.is_muted')
    ...
    public function node()
    {
        ...
        db_connect()
            ...
            ->insert([
                ...
                'description' => mb_substr((string) $this->request->getPost('description'), 0, 255),
    ...
    public function mute(int $id)
    {
        ...
        $db->table('user_profiles')
            ->where('user_id', $id)
            ->update(['is_muted' => $p['is_muted'] ? 0 : 1, 'updated_at' => gmdate('Y-m-d H:i:s')]);
        return redirect()
            ->back()
            ->with('success', $p['is_muted'] ? '已解除禁言。' : '已禁言。');
    }
    ...
    public function moderateComment(int $id)
    {
        ...
        try {
            $c = $db
                ->query(
                    'SELECT c.topic_id,c.user_id,c.status,t.status AS topic_status FROM comments c JOIN topics t ON t.id=c.topic_id WHERE c.id=? FOR UPDATE',
                    [$id],
                )
                ->getRowArray();
            if (!$c) {
