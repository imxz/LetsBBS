...
final class ForumModel extends Model
{
    ...
    {
        return $this->db->table('topics t')->select('t.*,n.name node_name,u.username,p.avatar')->join('nodes n', 'n.id=t.node_id')->join('users u', 'u.id=t.user_id')->join('user_profiles p', 'p.user_id=t.user_id', 'left')->where(['t.id' => $id,'t.status' => 'published'])->get()->getRowArray();
    }
    public function comments(int $topicId): array
    {
        return $this->db->table('comments c')->select('c.*,u.username,p.avatar')->join('users u', 'u.id=c.user_id')->join('user_profiles p', 'p.user_id=c.user_id', 'left')->where(['c.topic_id' => $topicId,'c.status' => 'published'])->orderBy('c.id')->get()->getResultArray();
    }
