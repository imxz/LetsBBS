        ...
        try {
            ...
            if (!$resuming) {
                $count =
                    (int) ($db
                        ->query(
                            "SELECT COUNT(*) AS n FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name <> 'migrations'",
                        )
                        ->getRowArray()['n'] ?? 0);
                if ($count > 0) {
                ...
                file_put_contents(
                    $statePath,
                    json_encode(['stage' => 'migrate', 'started_at' => gmdate(DATE_ATOM)]),
                    LOCK_EX,
                );
            }
            ...
            $db->table('user_profiles')
                ->ignore(true)
                ->insert(['user_id' => $existing->id, 'created_at' => $now, 'updated_at' => $now]);
            file_put_contents(
                WRITEPATH . 'install.lock',
                json_encode(['installed_at' => gmdate(DATE_ATOM), 'version' => '4.7.4'], JSON_UNESCAPED_SLASHES),
                LOCK_EX,
            );
