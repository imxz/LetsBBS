...
final class CreateForumTables extends Migration
{
    ...
    private function statements(): array
    {
        ...
        return [
            ...
            "CREATE TABLE topics (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, node_id INT UNSIGNED NOT NULL, user_id INT UNSIGNED NOT NULL, title VARCHAR(160) NOT NULL, body MEDIUMTEXT NOT NULL, status ENUM('published','hidden','deleted') NOT NULL DEFAULT 'published', comment_count INT UNSIGNED NOT NULL DEFAULT 0, follower_count INT UNSIGNED NOT NULL DEFAULT 0, last_activity_at DATETIME NOT NULL, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, CONSTRAINT fk_topics_node FOREIGN KEY (node_id) REFERENCES nodes(id) ON DELETE RESTRICT, CONSTRAINT fk_topics_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE RESTRICT, KEY idx_topics_list (status, last_activity_at DESC, id DESC), KEY idx_topics_node (node_id, status, last_activity_at DESC), KEY idx_topics_user (user_id, status, created_at DESC)) $suffix",
            ...
            "CREATE TABLE notifications (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, user_id INT UNSIGNED NOT NULL, actor_id INT UNSIGNED NULL, topic_id BIGINT UNSIGNED NULL, kind VARCHAR(32) NOT NULL, read_at DATETIME NULL, created_at DATETIME NOT NULL, CONSTRAINT fk_notifications_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, CONSTRAINT fk_notifications_actor FOREIGN KEY (actor_id) REFERENCES users(id) ON DELETE SET NULL, CONSTRAINT fk_notifications_topic FOREIGN KEY (topic_id) REFERENCES topics(id) ON DELETE CASCADE, KEY idx_notifications_inbox (user_id, read_at, id DESC)) $suffix",
